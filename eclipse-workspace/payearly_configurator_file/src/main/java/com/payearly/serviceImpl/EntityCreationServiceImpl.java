...
try {
	 ...
	 if (exception.getStatusCode().value()==400) {
		...
		entityDetails.setGstin(gstnResponseDTO.getGstin());
		 ...
		 entityDetails.setFlag(gstnResponseDTO.getFlag());
		 entityDetailsReposoitory.save(entityDetails);
...
//		HttpEntity<Map<Object, Object>> entity5 = new HttpEntity<Map<Object,Object>>(m,ut.createHeaders("jack", "jack"));
	...
	public void deleteEntityDetail(String getin) {
		...
		Optional<ProcessLog> processLog	= entityCreationRepository.findByGstin(getin);
		  ...
		  List<EntityAddress> entityAddresses = entityAddressRepository.findAllByEntityDetails_id(entityDetails.getId());
		  List<EntityMapping> entityMappings = entityMappingRepository.findByEntityDetails_id(entityDetails.getId());
		  if (entityMappings != null) {
			  entityMappings.stream().forEach(eMapping -> {
				  entityMappingRepository.delete(eMapping);
			  });
	     	}
		  if(entityAddresses != null) {
			  entityAddresses.stream().forEach(eAddress -> {
				  entityAddressRepository.delete(eAddress);
			  });
		  }
		  entityDetailsReposoitory.delete(entityDetails);
		  entityCreationRepository.deleteByGstin(getin);
	...
	public EntityMapping createEntityDetail(EntityMappingDTO entityAndAddressDetailDTO) {
		  EntityDetails  entityDetails =  entityDetailsReposoitory.findByentityName(entityAndAddressDetailDTO.getEntityName());
		
		   EntityMapping entityMapping = new EntityMapping();
		  // entityMapping.setProduct(entityAndAddressDetailDTO.getEntityMappingName());
		   entityMapping.setEntityDetails(entityDetails);
	...
	public List<EntityMapping> getEntityMapping(String entityName) {
		...
		return entityMappingRepository.findByentityDetails_entityName(entityName);
	}


	@Override
	public BankDetail saveBankDetails(BankDetail bankDetail) {
	...
	@Override
	public List<BankDetail> getBankDetailsById(String id) {
		return bankDetailRepository.findByEntityId(id);
		
	}


	@Override
	public void deleteBankDetailsByAccNo(Long id) {
